...
model System1
  ...
  Annex60.Fluid.HeatExchangers.Radiators.RadiatorEN442_2 rad(
    ...
    mDry=0.001, // no additional mass (mDry=0.0 doesn't work)
  ...
  Modelica.Blocks.Continuous.LimPID thermostat(
    ...
    k=0.125, // IDA model Tset = 22°C, DeadBand = 2 K -> y = 0.5 * (u_s - u_m)
  ...
  Annex60.Fluid.Movers.FlowControlled_dp pump(
    redeclare package Medium = Medium,
    m_flow_nominal=m_flow_nominal)
...
equation
  ...
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-100,-80},{100,60}}),graphics), 
     experiment(StartTime=0, StopTime=31536000, Tolerance = 1e-4),
